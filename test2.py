...
life_length_per = round(len(np.where(static_result['生命线指宽'].values < (life_length / finger_width))[0]) / 5502 * 100,2)
life_over_per = round(len(np.where(static_result['生命线是否过中线'].values == life_over)[0]) / 5502 * 100,2)
int_length_per = round(len(np.where(static_result['智慧线指宽'].values < (int_length / finger_width))[0]) / 5502 * 100,2)
...
aff_length_per = round(len(np.where(static_result['感情线指宽'].values < (aff_length / finger_width))[0]) / 5502 * 100,2)
...
life_length_des,life_over_des,int_length_des,int_over_des,int_dis_des,aff_length_des,aff_over_des,aff_dis_des,aff_curve_des=fortune_telling(life_length_per,life_over,int_length_per,int_over,int_dis_per,aff_length_per,aff_over,aff_dis_per,aff_curve_per)

data_all = []
data_all.append(['生命线长度',str(round(life_length/cm_std,2))+'cm','/',str(life_length_per)+'%',life_length_des])
...
data_all.append(['智慧线长度',str(round(int_length/cm_std,2))+'cm','/',str(int_length_per)+'%',int_length_des])
...
data_all.append(['感情线长度',str(round(aff_length/cm_std))+'cm','/',str(aff_length_per)+'%',aff_length_des])
